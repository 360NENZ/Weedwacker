...
using Weedwacker.WebServer.Event;
/*
namespace Weedwacker.WebServer.Plugin
{
    ...
    internal class PluginManager
    {
        ...
        /// </summary>
        private readonly Dictionary<Plugin, List<Server.Event.EventHandler<Event>>> listeners = new();
        ...
        /// </summary>
        private static struct PluginData
        {
        ...
        private void loadPlugins()
        {
            ...
            {
                Logger.WriteErrorLine("Failed to create plugins directory: " + pluginsDir.getAbsolutePath());
                return;
            }
            ...
            List<PluginData> dependencies = new();

            // Initialize all plugins.
            foreach (File plugin in plugins)
            {
                try
                {
                    ...
                    {
                        Logger().warn("Plugin " + plugin.getName() + " has an invalid main class.");
                    }
                    catch (FileNotFoundException ignored)
                    {
                        Logger().warn("Plugin " + plugin.getName() + " lacks a valid config file.");
                    }
                } 
                catch (Exception exception)
                {
                    Logger().error("Failed to load plugin: " + plugin.getName(), exception);
                }
           }
    ...
    if (depth >= maxDepth)
    {
        Logger().error("Failed to load plugins with dependencies.");
        break;
    }
    ...
    try
    {
        ...
        {
        Logger().error("Failed to load a plugin.", exception); depth++;
        }
        }
    }
    ...
    {
    Logger().info("Loading plugin: " + identifier.name);
    ...
    /// <param name="plugin"> The plugin registering the listener.</param>
...
public void invokeEvent(Event event)
    ...
    /// <param name="event"> The event being called.</param>
    /// <param name="priority"> The priority to call for.</param>
    /// </summary>
...
/// <returns>Either null, or the plugin's instance.</returns>
/// </summary>
@Nullable
...
public void enablePlugin(Plugin plugin)
    ...
    {
        Logger().error("Failed to enable plugin: " + plugin.getName(), exception);
    }
    }
...
public void disablePlugin(Plugin plugin)
    ...
    {
        Logger().error("Failed to disable plugin: " + plugin.getName(), exception);
    }
...
/// <param name="event"> The event passed through to the handler.</param>
